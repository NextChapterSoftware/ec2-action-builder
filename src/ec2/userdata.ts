...
export class UserData {
  ...
  async getUserData(): Promise<string> {
    ...
    const cmds = [
      ...
      "SHUTDOWN_SCRIPT_PATH=$CURRENT_PATH/shutdown_script.sh",
      `echo "./config.sh remove --token ${runnerRegistrationToken.token} || true" > $SHUTDOWN_SCRIPT_PATH`,
      `echo "shutdown -P +1" > $SHUTDOWN_SCRIPT_PATH`,
      "chmod +x $SHUTDOWN_SCRIPT_PATH",
      `echo "./config.sh remove --token ${runnerRegistrationToken.token} || true" > $SHUTDOWN_SCRIPT_PATH`,
      `echo "shutdown -h now" > $SHUTDOWN_SCRIPT_PATH`,
      "chmod +x $SHUTDOWN_SCRIPT_PATH",
      "export ACTIONS_RUNNER_HOOK_JOB_COMPLETED=$SHUTDOWN_SCRIPT_PATH",
      "mkdir -p actions-runner && cd actions-runner",
      'echo "ACTIONS_RUNNER_HOOK_JOB_COMPLETED=$SHUTDOWN_SCRIPT_PATH" > .env',
